...
data_to_dump = pd.concat([X_train, X_test])
data_to_dump.to_csv("H:/ai_training_named.csv", index = False)
#%%
param_grid = { 
...
#%%
good = all_annotated.loc[all_annotated['label_str'] != 'good'].copy()
...
wp = load_worm_predictor(r"C:\Users\ebjam\Documents\GitHub\wormfind\models\worm_detection_classifier\best_sato_Oct24_noZscore.joblib")
...
X_test['area_to_perimeter'] = X_test['area'] / X_test['perimeter']
y_pred = wp.predict(X_test)

# Calculate accuracy
accuracy = accuracy_score(y_test, y_pred)

print(f"Accuracy of the loaded random forest: {accuracy:.4f}")
