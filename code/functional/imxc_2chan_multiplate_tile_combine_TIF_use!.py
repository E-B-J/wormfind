...
base_path = "E:/toronto_microscopy/ixmc/Sep16_detergent_tests/Sep16_p1/"
...
for plate in plates:
    ...
    fish_path = plate_path + "fish/"
    chan_paths = [dapi_path, dy96_path, fish_path]
...
w3_s1_images = [r[:-3] + 'TIF' for r in w3_imgs if r[-8] == '1']
...
for top_left in w3_s1_images:
    ...
    s1 = cv2.imread(os.path.join(fish_path + top_left), -1)
    s2 = cv2.imread(os.path.join(fish_path + tr), -1)
    s3 = cv2.imread(os.path.join(fish_path + bl), -1)
    s4 = cv2.imread(os.path.join(fish_path + br), -1)


    top_row = cv2.hconcat([s1, s2])
    bottom_row = cv2.hconcat([s3, s4])
    combined_image = cv2.vconcat([top_row, bottom_row])
    cv2.imwrite(fish_path + 'one_field/' + top_left[:-10] + 'w1_combo.TIF', combined_image)
